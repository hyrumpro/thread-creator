...
export default function Profile() {
  ...
  const tweetsQuery = useUserTweets(isOwnProfile ? currentUser.id : fetchedProfile?.user_id);
  const isTweetsLoading = tweetsQuery.isLoading;

  const { data: isFollowing } = useUserFollowStatus(
    isOwnProfile ? undefined : fetchedProfile?.user_id
  );
  ...
  const displayUser = useMemo<User>(() => {
    ...
    if (fetchedProfile) {
      return {
        ...
        avatar: fetchedProfile.avatar || 'https://api.dicebear.com/7.x/avataaars/svg?seed=' + fetchedProfile.username,
        bio: fetchedProfile.bio || undefined,
        followers: fetchedProfile.followers_count,
        following: fetchedProfile.following_count,
        isVerified: fetchedProfile.is_verified,
        isPro: fetchedProfile.is_pro,
        joinedDate: new Date(fetchedProfile.created_at).toLocaleString("en-US", {
          month: "long",
          year: "numeric",
        }),
        coverImage: fetchedProfile.cover_image || undefined,
      };
    }
    return currentUser;
  ...
  const tweets = useMemo(() => {
    if (!tweetsQuery.data?.pages) return [];
    return tweetsQuery.data.pages.flatMap((page) => page.items);
  }, [tweetsQuery.data]);
  ...
  const handleFollow = async () => {
    if (!fetchedProfile?.user_id) return;
    setIsFollowLoading(true);
    try {
      if (isFollowing) {
        await profileService.unfollowUser(fetchedProfile.user_id);
        toast({ title: `Unfollowed @${fetchedProfile.username}` });
      } else {
        await profileService.followUser(fetchedProfile.user_id);
        toast({ title: `Following @${fetchedProfile.username}` });
      }
  ...
  return (
    <MainLayout>
      <header className="sticky top-0 z-10 bg-background/80 backdrop-blur-md border-b border-border">
        <div className="flex items-center gap-6 p-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">{tweets.length} posts</p>
          </div>
        </div>
      </header>
      ...
      <TweetFeed
        ...
        isInitialLoading={isTweetsLoading}
        isFetchingMore={tweetsQuery.isFetchingNextPage}
        onLoadMore={tweetsQuery.hasNextPage ? tweetsQuery.fetchNextPage : undefined}
