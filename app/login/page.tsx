...
import { Eye, EyeOff, Loader2, Mail, AlertCircle, RefreshCw } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { authService } from "@/lib/auth";
import { showErrorToast, showSuccessToast, isNetworkError } from "@/lib/error-handler";
...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  "Sign in not allowed": "Sign in is not allowed for this account. Please contact support.",
...
export default function Login() {
  ...
  const [errorType, setErrorType] = useState<'error' | 'network' | null>(null);
  ...
  const handleMagicLink = async () => {
    ...
    try {
      ...
      showSuccessToast("Magic link sent!", "Check your email for the login link.");
    } catch (err: any) {
            ...
            <div className={`mb-6 p-4 rounded-lg flex items-start gap-3 ${
              ...
              <div className="flex-1">
                <p className={`text-sm font-medium ${
                  errorType === 'network' ? 'text-yellow-500' : 'text-destructive'
                }`}>
                  {errorType === 'network' ? 'Connection Error' : 'Sign in failed'}
                </p>
